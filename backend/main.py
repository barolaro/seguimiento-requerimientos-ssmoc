...
def me(user: dict[str, Any] = Depends(current_user)) -> dict[str, Any]:
    return {k: user.get(k, "") for k in ("usuario", "nombre", "email", "rol")}
...
def update_requirement(
    ...
    can_edit = user.get("rol") != "Ejecutivo" or item.get("responsable") == user["usuario"]
    if not can_edit:
...
def list_users(user: dict[str, Any] = Depends(require_management)) -> list[dict[str, Any]]:
    return [
        {k: item.get(k, "") for k in ("usuario", "nombre", "email", "rol", "activo")}
