/* SGTCP 3.1.1 · Performance + UX layer */
(function(){
  ...
  function schedulePrefetch(){const fn=()=>prefetchRecent();if('requestIdleCallback'in window)requestIdleCallback(fn,{timeout:3000});else setTimeout(fn,1800)}
  ...
  window.openDetail=openDetail=async function(id){
    ...
    try{const payload=await fetchDetail(id,false);if(Number(state.selectedId)===Number(id))$('timeline').innerHTML=timelineHtml(payload.eventos||[],r)}catch(e){if(!cached&&Number(state.selectedId)===Number(id))$('timeline').innerHTML=timelineHtml([],r)+`<p class="error">No se pudo refrescar el historial: ${e.message}</p>`}
  };
  ...
  window.saveUpdate=saveUpdate=async function(){
    ...
    const snapshot={...r};
    const oldState=r.estado,oldAdvance=Number(r.avance||0),now=new Date().toISOString();
    const cached=getCache(r.id)||{eventos:[]};
    const optimistic={fecha:now,tipo:oldState!==cambios.estado?'estado':oldAdvance!==cambios.avance?'avance':'actualizacion',autor:state.user?.usuario,detalle,estado_anterior:oldState,estado_nuevo:cambios.estado,responsable_anterior:r.responsable,responsable_nuevo:r.responsable,avance_anterior:oldAdvance,avance_nuevo:cambios.avance,_pending:true};

    /* RESPUESTA INSTANTÁNEA: primero actualizamos la interfaz. */
    Object.assign(r,cambios,{actualizado:now});
    cached.eventos=(cached.eventos||[]).concat([optimistic]);
    cached.requerimiento={...r};
    saveCache(r.id,cached);
    $('detailUpdate').value='';
    $('timeline').innerHTML=timelineHtml(cached.eventos,r);
    setLockedUI(r);
    renderAll();
    $('detailDialog').close();
    toast(`REQ-${String(r.id).padStart(3,'0')} actualizado · sincronizando con Google Sheets…`,'success');

    /* Google Sheets guarda en segundo plano. El ejecutivo puede seguir trabajando. */
    try{
      await call('actualizar_req',{id:r.id,cambios,detalle:detail});
      optimistic._pending=false;
      cached.requerimiento={...r};
      saveCache(r.id,cached);
      toast(`✓ REQ-${String(r.id).padStart(3,'0')} guardado correctamente.`,'success');
    }catch(e){
      Object.keys(r).forEach(k=>delete r[k]);Object.assign(r,snapshot);
      cached.eventos=(cached.eventos||[]).filter(x=>x!==optimistic);
      cached.requerimiento={...r};
      saveCache(r.id,cached);
      renderAll();
      toast(`No se pudo sincronizar REQ-${String(r.id).padStart(3,'0')}. Se revirtió el cambio: ${e.message}`,'error');
    }
  };
  ...
  if(window.Chart){Chart.defaults.animation.duration=60;Chart.defaults.responsive=true}document.addEventListener('DOMContentLoaded',schedulePrefetch);
})();