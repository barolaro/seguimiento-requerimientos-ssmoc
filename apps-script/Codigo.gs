/**
 * SGTCP 3.2 · Backend Google Apps Script optimizado
 * Servicio de Salud Metropolitano Occidente
 */

const APP_VERSION = '3.2.0';
const SESSION_SECONDS = 21600;
const SCHEMA_CACHE_KEY = 'sgtcp_schema_ok_320';
const SCHEMA_CACHE_SECONDS = 3600;
...
function configurarProduccion() {
  ...
  invalidateUserCache_('bayron.admin');
...
/**
 * D-07: antes se revisaban cabeceras de cuatro hojas en CADA request.
 * Ahora solo se valida una vez por hora por instancia de Apps Script.
 */
function ensureSchema_() {
  const cache = CacheService.getScriptCache();
  if (cache.get(SCHEMA_CACHE_KEY) === '1') return;

  ensureSheet_(SH_USERS, USERS_COLS);
  ensureSheet_(SH_REQ, REQ_COLS);
  ensureSheet_(SH_EVENTS, EVENT_COLS);
  ensureSheet_(SH_AUDIT, AUDIT_COLS);
...
function login_(username, password) {
  ...
  updateRowByKey_(sheetFast_(SH_USERS), USERS_COLS, 'usuario', user.usuario, {ultimo_acceso:now_()});
  invalidateUserCache_(user.usuario);
...
function logout_(token, ctx) {
  CacheService.getScriptCache().remove('sess:' + token);
  audit_(ctx.user.usuario,'logout','sesion','', 'Cierre de sesión');
  return {ok:true};
}
...
function requireSession_(token) {
  ...
  const user = getUser_(payload.usuario);
...
function cambiarPassword_(ctx, actual, nueva) {
  ...
  updateRowByKey_(sheetFast_(SH_USERS), USERS_COLS, 'usuario', ctx.user.usuario, {password_hash:hashPassword_(nueva), cambiar_password:'NO'});
  invalidateUserCache_(ctx.user.usuario);
  audit_(ctx.user.usuario,'cambiar_password','usuario',ctx.user.usuario,'Contraseña modificada');
  return {ok:true};
}

function listar_(ctx) {
  const allReq = readObjects_(sheetFast_(SH_REQ)).map(normalizeReq_);
  const visible = isExecutive_(ctx.user) ? allReq.filter(r => String(r.responsable) === String(ctx.user.usuario)) : allReq;
  const users = isExecutive_(ctx.user) ? [publicUser_(ctx.user)] : readObjects_(sheetFast_(SH_USERS)).map(publicUser_);
...
function detalle_(ctx, id) {
  ...
  authorizeReq_(ctx.user, req);
  const events = readObjects_(sheetFast_(SH_EVENTS))
    .filter(e => Number(e.requerimiento_id) === Number(id))
    .map(normalizeEvent_)
    .sort((a,b) => String(a.fecha).localeCompare(String(b.fecha)));
  return {ok:true, data:{requerimiento:normalizeReq_(req), eventos:events}};
}

function obtenerAuditoria_(ctx, limit) {
  requireAdmin_(ctx.user);
  const rows = readObjects_(sheetFast_(SH_AUDIT)).map(normalizeAudit_);
  return {ok:true, data:rows.slice(-Math.min(Math.max(limit,1),1000)).reverse()};
}
...
function crearReq_(ctx, req) {
  ...
  lock.waitLock(10000);
  try {
    const sh = sheetFast_(SH_REQ);
    const rows = readObjects_(sh);
    const id = rows.reduce((m,r) => Math.max(m, Number(r.id)||0), 0) + 1;
    const now = now_();
    const item = {
      id:id, titulo:String(req.titulo).trim(), descripcion:String(req.descripcion || '').trim(),
      responsable:responsible.usuario, estado:'Pendiente', prioridad:String(req.prioridad || 'Media'), avance:0,
      creado:now, compromiso:normalizeDateOnly_(req.compromiso), creado_por:ctx.user.usuario, actualizado:now
    };
    appendObject_(sh, REQ_COLS, item);
    event_(id,'asignacion',ctx.user.usuario,'Requerimiento creado y asignado.','','Pendiente','',responsible.usuario,'',0);
    audit_(ctx.user.usuario,'crear_requerimiento','requerimiento',String(id),item.titulo);
    return {ok:true, id:id, requerimiento:normalizeReq_(Object.assign({}, item))};
  } finally {
    lock.releaseLock();
  }
}
...
function actualizarReq_(ctx, id, changes, detail) {
  ...
  const clean = {};
  allowed.forEach(k => { if (changes[k] !== undefined) clean[k] = changes[k]; });
  if (clean.avance !== undefined) clean.avance = Math.max(0, Math.min(100, Number(clean.avance)||0));
  if (clean.compromiso !== undefined) clean.compromiso = normalizeDateOnly_(clean.compromiso);
  if (String(clean.estado) === 'Terminado') clean.avance = 100;
  clean.actualizado = now_();

  const oldState = req.estado;
  const oldAdvance = Number(req.avance)||0;
  updateRowByKey_(sheetFast_(SH_REQ), REQ_COLS, 'id', Number(id), clean);

  const newState = clean.estado !== undefined ? clean.estado : req.estado;
  const newAdvance = clean.avance !== undefined ? clean.avance : oldAdvance;
  ...
  audit_(ctx.user.usuario,'actualizar_requerimiento','requerimiento',String(id),String(detail || type));

  const updated = Object.assign({}, req, clean);
  return {ok:true, requerimiento:normalizeReq_(updated)};
}
...
function reasignarReq_(ctx, id, newResponsible, reason) {
  ...
  const now = now_();
  updateRowByKey_(sheetFast_(SH_REQ), REQ_COLS, 'id', Number(id), {responsable:target.usuario, actualizado:now});
  ...
  audit_(ctx.user.usuario,'reasignar_requerimiento','requerimiento',String(id),previous + ' → ' + target.usuario);
  return {ok:true, requerimiento:normalizeReq_(Object.assign({}, req, {responsable:target.usuario, actualizado:now}))};
}
...
function eliminarReq_(ctx, id, reason) {
  ...
  deleteRowByKey_(sheetFast_(SH_REQ), REQ_COLS, 'id', Number(id));
  return {ok:true};
}

function crearUsuario_(ctx, u) {
  requireAdmin_(ctx.user);
  const username = String(u.usuario || '').toLowerCase().trim();
  ...
  const temp = String(u.password || tempPassword_());
  if (temp.length < 10) throw new Error('La contraseña inicial debe tener al menos 10 caracteres.');

  appendObject_(sheetFast_(SH_USERS), USERS_COLS, {
    usuario:username, nombre:String(u.nombre || '').trim(), email:String(u.email || '').trim(), rol:String(u.rol),
    password_hash:hashPassword_(temp), activo:'SI', cambiar_password:'SI', ultimo_acceso:'', creado:now_()
  });
  invalidateUserCache_(username);
  audit_(ctx.user.usuario,'crear_usuario','usuario',username,String(u.rol));
  return {ok:true, usuario:username, password_temporal:u.password ? undefined : temp};
}

function actualizarUsuario_(ctx, username, changes) {
  requireAdmin_(ctx.user);
  const user = getUser_(username);
  if (!user) throw new Error('Usuario no encontrado.');
  const clean = {};
  ['nombre','email','rol','activo'].forEach(k => { if (changes[k] !== undefined) clean[k] = changes[k]; });
  if (clean.activo !== undefined) clean.activo = bool_(clean.activo) ? 'SI' : 'NO';
  updateRowByKey_(sheetFast_(SH_USERS), USERS_COLS, 'usuario', user.usuario, clean);
  invalidateUserCache_(user.usuario);
  audit_(ctx.user.usuario,'actualizar_usuario','usuario',user.usuario,JSON.stringify(clean));
  return {ok:true};
}
...
function resetPassword_(ctx, username) {
  ...
  if (!user) throw new Error('Usuario no encontrado.');
  const temp = tempPassword_();
  updateRowByKey_(sheetFast_(SH_USERS), USERS_COLS, 'usuario', user.usuario, {password_hash:hashPassword_(temp), cambiar_password:'SI'});
  invalidateUserCache_(user.usuario);
  audit_(ctx.user.usuario,'reset_password','usuario',user.usuario,'Contraseña temporal generada');
  return {ok:true, password_temporal:temp};
}
...
/** Acceso rápido: no verifica cabeceras en cada llamada. */
function sheetFast_(name) {
  const sh = ss_().getSheetByName(name);
  if (!sh) throw new Error('Falta la hoja requerida: ' + name + '. Ejecute configurarProduccion().');
  return sh;
}

/** Solo se usa durante validación de esquema. */
function ensureSheet_(name, cols) {
  ...
  if (sh.getLastRow() === 0) {
    ...
    const head = sh.getRange(1,1,1,width).getValues()[0].slice(0,cols.length);
    if (head.join('|') !== cols.join('|')) sh.getRange(1,1,1,cols.length).setValues([cols]);
  }
  return sh;
}
...
function appendObject_(sh, cols, obj) {
  sh.appendRow(cols.map(c => obj[c] !== undefined ? obj[c] : ''));
}

/**
 * Busca SOLO en la columna clave y lee la fila completa únicamente al encontrarla.
 * Evita getDataRange() completo para getUser_, getReq_, update y delete.
 */
function findRowByKey_(sh, cols, key, value) {
  const keyIdx = cols.indexOf(key);
  if (keyIdx < 0) throw new Error('Columna clave inexistente: ' + key);
  const lastRow = sh.getLastRow();
  if (lastRow < 2) return null;
  const keys = sh.getRange(2, keyIdx + 1, lastRow - 1, 1).getValues();
  for (let i=0; i<keys.length; i++) {
    if (String(keys[i][0]).toLowerCase() === String(value).toLowerCase()) {
      const rowNumber = i + 2;
      const values = sh.getRange(rowNumber,1,1,cols.length).getValues()[0];
      const obj={};
      cols.forEach((c,j)=>obj[c]=values[j]);
      return {rowNumber:rowNumber, values:values, object:obj};
    }
  }
  return null;
}
...
function updateRowByKey_(sh, cols, key, value, changes) {
  const found = findRowByKey_(sh, cols, key, value);
  if (!found) throw new Error('Registro no encontrado: ' + value);
  const row = found.values.slice();
  Object.keys(changes).forEach(k => {
    const idx = cols.indexOf(k);
    if (idx >= 0) row[idx] = changes[k];
  });
  sh.getRange(found.rowNumber,1,1,cols.length).setValues([row]);
}

function deleteRowByKey_(sh, cols, key, value) {
  const found = findRowByKey_(sh, cols, key, value);
  if (!found) throw new Error('Registro no encontrado: ' + value);
...
function getUser_(username) {
  ...
  const found = findRowByKey_(sheetFast_(SH_USERS), USERS_COLS, 'usuario', u);
  if (!found) return null;
  const user = found.object;
  try { cache.put(ck, JSON.stringify(user), USER_CACHE_SECONDS); } catch (_) {}
  return user;
}

function invalidateUserCache_(username) {
  CacheService.getScriptCache().remove('user:' + String(username||'').toLowerCase().trim());
}

function getReq_(id) {
  const found = findRowByKey_(sheetFast_(SH_REQ), REQ_COLS, 'id', Number(id));
  if (!found) throw new Error('Requerimiento no encontrado.');
  return normalizeReq_(found.object);
}

/** D-04: normalización consistente de fechas provenientes de Sheets. */
function normalizeDateOnly_(v) {
  if (!v) return '';
  if (v instanceof Date) return Utilities.formatDate(v, Session.getScriptTimeZone() || 'America/Santiago', 'yyyy-MM-dd');
  const s = String(v).trim();
  if (/^\d{4}-\d{2}-\d{2}$/.test(s)) return s;
  const d = new Date(v);
  return isNaN(d.getTime()) ? s : Utilities.formatDate(d, Session.getScriptTimeZone() || 'America/Santiago', 'yyyy-MM-dd');
}

function normalizeDateTime_(v) {
  if (!v) return '';
  if (v instanceof Date) return Utilities.formatDate(v, Session.getScriptTimeZone() || 'America/Santiago', "yyyy-MM-dd'T'HH:mm:ssXXX");
  return String(v);
}

function normalizeReq_(r) {
  r.id = Number(r.id)||0;
  r.avance = Number(r.avance)||0;
  r.compromiso = normalizeDateOnly_(r.compromiso);
  r.creado = normalizeDateTime_(r.creado);
  r.actualizado = normalizeDateTime_(r.actualizado);
  return r;
}

function normalizeEvent_(e) {
  e.fecha = normalizeDateTime_(e.fecha);
  return e;
}

function normalizeAudit_(a) {
  a.fecha = normalizeDateTime_(a.fecha);
  return a;
}

function publicUser_(u) {
  return {
    usuario:u.usuario, nombre:u.nombre, email:u.email, rol:u.rol,
    activo:bool_(u.activo), cambiar_password:bool_(u.cambiar_password),
    ultimo_acceso:normalizeDateTime_(u.ultimo_acceso)
  };
}
...
function event_(reqId,type,author,detail,oldState,newState,oldResp,newResp,oldAdvance,newAdvance) {
  appendObject_(sheetFast_(SH_EVENTS), EVENT_COLS, {
    evento_id:'EV-'+new Date().getTime()+'-'+Math.floor(Math.random()*1000),
    requerimiento_id:Number(reqId), fecha:now_(), tipo:type, autor:author, detalle:detail,
    estado_anterior:oldState||'', estado_nuevo:newState||'',
    responsable_anterior:oldResp||'', responsable_nuevo:newResp||'',
    avance_anterior:oldAdvance===''?'':Number(oldAdvance||0), avance_nuevo:newAdvance===''?'':Number(newAdvance||0)
  });
}

function audit_(user,action,entity,entityId,detail) {
  appendObject_(sheetFast_(SH_AUDIT), AUDIT_COLS, {
    evento_id:'AU-'+new Date().getTime()+'-'+Math.floor(Math.random()*1000),
    fecha:now_(), usuario:user||'', accion:action, entidad:entity,
    entidad_id:entityId||'', detalle:detail||''
  });
}
