 ...
 *
 * Producción sin token maestro en el navegador.
 * - Login genera una sesión temporal (6 horas).
 * - Cada acción valida usuario activo y rol.
 * - Ejecutivos solo leen/modifican requerimientos propios.
 * - Jefaturas gestionan asignaciones y ven el equipo.
 * - Administrador gestiona usuarios y ve toda la auditoría.
 * - La trazabilidad se almacena como eventos inmutables.
 */
...
function configurarProduccion() {
  ...
  appendObject_(sheet_(SH_USERS, USERS_COLS), USERS_COLS, {
    usuario:'bayron.admin',
    nombre:'Bayron Retamal González',
    email:'',
    rol:'Administrador',
  ...
  Logger.log('Cambie la contraseña en el primer ingreso.');
}

function ensureSchema_() {
  sheet_(SH_USERS, USERS_COLS);
  sheet_(SH_REQ, REQ_COLS);
  sheet_(SH_EVENTS, EVENT_COLS);
  sheet_(SH_AUDIT, AUDIT_COLS);
  const props = PropertiesService.getScriptProperties();
...
function login_(username, password) {
  ...
  return {
    ok:true,
    session:token,
    cambiar_password:bool_(user.cambiar_password),
...
function cambiarPassword_(ctx, actual, nueva) {
  ...
  updateRowByKey_(sheet_(SH_USERS, USERS_COLS), USERS_COLS, 'usuario', ctx.user.usuario, {
    password_hash:hashPassword_(nueva), cambiar_password:'NO'
  });
...
function listar_(ctx) {
  ...
  const visible = isExecutive_(ctx.user)
    ? allReq.filter(r => String(r.responsable) === String(ctx.user.usuario))
    : allReq;
  const users = isExecutive_(ctx.user)
    ? [publicUser_(ctx.user)]
    : readObjects_(sheet_(SH_USERS, USERS_COLS)).map(publicUser_);
...
function detalle_(ctx, id) {
  ...
  const events = readObjects_(sheet_(SH_EVENTS, EVENT_COLS))
...
function obtenerAuditoria_(ctx, limit) {
  ...
  const rows = readObjects_(sheet_(SH_AUDIT, AUDIT_COLS));
...
function crearReq_(ctx, req) {
  ...
  try {
    const rows = readObjects_(sheet_(SH_REQ, REQ_COLS));
    ...
    const item = {
      ...
      creado:now, compromiso:String(req.compromiso || ''), creado_por:ctx.user.usuario, actualizado:now
    };
...
function actualizarUsuario_(ctx, username, changes) {
  requireAdmin_(ctx.user);
  const user = getUser_(username); if (!user) throw new Error('Usuario no encontrado.');
  const clean = {};
  ...
  updateRowByKey_(sheet_(SH_USERS, USERS_COLS), USERS_COLS, 'usuario', user.usuario, clean);
...
function resetPassword_(ctx, username) {
  ...
  const user = getUser_(username); if (!user) throw new Error('Usuario no encontrado.');
  const temp = tempPassword_();
  updateRowByKey_(sheet_(SH_USERS, USERS_COLS), USERS_COLS, 'usuario', user.usuario, {password_hash:hashPassword_(temp), cambiar_password:'SI'});
...
function sheet_(name, cols) {
  ...
  if (sh.getLastRow() === 0) sh.appendRow(cols);
    ...
    const head = sh.getRange(1,1,1,Math.max(sh.getLastColumn(),cols.length)).getValues()[0].slice(0,cols.length);
...
function readObjects_(sh) {
  ...
  const head = values[0];
  return values.slice(1).filter(r => r.some(v => v !== '')).map(r => { const o={}; head.forEach((h,i)=>o[h]=r[i]); return o; });
}
...
function updateRowByKey_(sh, cols, key, value, changes) {
  const values = sh.getDataRange().getValues(); const keyIdx = cols.indexOf(key);
  for (let i=1;i<values.length;i++) {
    if (String(values[i][keyIdx]) === String(value)) {
      Object.keys(changes).forEach(k => { const idx=cols.indexOf(k); if(idx>=0) values[i][idx]=changes[k]; });
      sh.getRange(i+1,1,1,cols.length).setValues([values[i].slice(0,cols.length)]); return;
    }
  }
  throw new Error('Registro no encontrado: ' + value);
}
