...
# Capa responsive universal. Evita escalar por resolución física (4K, Full HD, etc.)
# y trabaja con el ancho/alto CSS real disponible en el navegador.
universal_css = r"""
...
@media (min-width:821px){
  ...
  .topbar nav button{padding:8px 9px;font-size:9px;line-height:1.1}
  .account{min-width:max-content}
  .account b{font-size:10px}.account span{font-size:8px}
  .account .btn{padding:10px 13px}

  main{width:min(1480px,calc(100% - 40px));padding:28px 0 56px}
}

/* Escritorio intermedio: menú en segunda fila, sin scroll horizontal */
...
/* Escritorios amplios: crecimiento muy moderado, nunca gigante */
@media (min-width:1900px){
  main{width:min(1720px,calc(100% - 72px));}
  .page-head h2{font-size:34px}
  .kpi strong{font-size:31px}
  .panel-head h3{font-size:15px}
}

/* Evita cualquier scrollbar horizontal del header en escritorio */
@media (min-width:821px){
...
config_script = "<script>window.SGTCP_CONFIG=" + json.dumps(
    {"appsScriptUrl": apps_script_url, "version": "3.1.0"}, ensure_ascii=False
) + ";</script>"
...
html = html.replace(
    ...
    'SGTCP 3.1.0 · Responsive universal<br>Diseñado y desarrollado por <strong>Bayron Retamal González</strong></small>'
)
for old in (
    'SGTCP 3.0 · Diseñado y desarrollado',
    ...
    'SGTCP 3.0.7 · Diseñado y desarrollado',
    'SGTCP 3.0.8 · Diseñado y desarrollado',
    'SGTCP 3.0.9 · Diseñado y desarrollado',
):
    html = html.replace(old, 'SGTCP 3.1.0 · Diseñado y desarrollado')

# Un visor estable evita que el login se centre en un iframe excesivamente alto.
# El contenido largo del dashboard se desplaza dentro del visor.
